    ...
    $(document).ready(function () {
        ...
        overrideWinOpen();
            ...
            $("#loginO365PopupButton").click(function () {
                var url = "https://localhost:6000/requestToken";
                showLoginPopup(url);
            });
...
function createAccountsTable(accounts) {
    ...
    Excel.run(function (ctx) {
        ...
        // Create a proxy object for the table rows
        var tableRows = table.rows;
        var r = tableRows.add(null, [[accounts.email, accounts.email, accounts.email, accounts.email, accounts.email]]);

        // $.each(accounts, function (i, item) {
        //     var r = tableRows.add(null, [[item.email, item.email, item.email, item.email, item.email]]);
        //     r.getRange().numberFormat = [[null, null, null, null, '_($* #,##0.00_);_($* (#,##0.00);_($* "-"??_);_(@_)']];
        //     r.getRange().format.autofitColumns();
